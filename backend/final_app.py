...
@app.get("/test")
def test_endpoint():
    return {"status": "working", "message": "Backend is running correctly", "models_loaded": models_loaded}
...
def generate_description(product: ProductID):
    ...
    try:
        ...
        if not models_loaded:
            return {"generated": f"This is a mock generated description for product {product.product_id}."}
        ...
        description = f"This {title} by {brand} is a high-quality product. "
        if category:
            description += f"It belongs to the {category} category. "
        if material:
            description += f"Made from {material}. "
        if color:
            description += f"Available in {color}. "
        description += "Perfect for your home and lifestyle needs."
...
if __name__ == "__main__":
    ...
    uvicorn.run("final_app:app", host="0.0.0.0", port=8002, reload=True)
